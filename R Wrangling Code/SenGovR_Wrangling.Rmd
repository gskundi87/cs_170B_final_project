...
data2018 = read.spss("/RawSPSSPollFiles/SenPolls/31116394_National 2018.por", to.data.frame = TRUE)
...
data2014 = read.spss("/RawSPSSPollFiles/SenPolls/31115091_National_2014.por", to.data.frame = TRUE)
...
data2006 = read.spss("/RawSPSSPollFiles/SenPolls/mi2006natelec.por", to.data.frame = TRUE)
...
data2002 = read.spss("/RawSPSSPollFiles/SenPolls/vns2002natelec.por", to.data.frame = TRUE)
...
data2000 = read.spss("/RawSPSSPollFiles/SenPolls/vns2000natelec.por", to.data.frame = TRUE)
...
data1996 = read_por("/RawSPSSPollFiles/SenPolls/usvns1996-natelec.por")
data1996 <- data1996 %>% 
  mutate('SEN' = case_when(
    QI == 1 ~ "D",
    QI == 2 ~ "R",
...
data1994 = read_por("/RawSPSSPollFiles/SenPolls/usvns1994-natelec.por")
data1994 <- data1994 %>% 
  mutate('SEN' = case_when(
    QC == 1 ~ "D",
    QC == 2 ~ "R",
...
data1992 = read_por("/RawSPSSPollFiles/SenPolls/vrs1992natelec.por")
data1992 <- data1992 %>% 
  mutate('SEN' = case_when(
    QIW_QHP == 1 ~ "D",
    QIW_QHP == 2 ~ "R",
...
data1990 = read_por("/RawSPSSPollFiles/SenPolls/vrs1990natelec.por")
data1990 <- data1990 %>% 
  mutate('SEN' = case_when(
    QH_QI == 1 ~ "D",
    QH_QI == 2 ~ "R",
